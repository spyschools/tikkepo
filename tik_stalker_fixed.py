...
def parse_tiktoker_data(username, field: dict):
    user_data = field.get("userInfo", {}).get("user", {})
    user_stats = field.get("userInfo", {}).get("stats", {})
    user_share_meta = field.get("shareMeta", {})
    ...
    print(f'{RED}Bios:{GREEN}             {signature if signature else "N/A"}')
    print(f'{RED}Private Account:{GREEN}  {user_data.get("privateAccount", "N/A")}')
    print(f'{RED}User Country:{GREEN}     {user_data.get("region", "N/A")}')
    print(f'{RED}Account Language:{GREEN} {user_data.get("language", "N/A")}')

    # Account Status
    print(f'{RED}\nTotal Followers:{GREEN}  {user_stats.get("followerCount", "N/A")}')
    print(f'{RED}Total Following:{GREEN}  {user_stats.get("followingCount", "N/A")}')
    print(f'{RED}Total Hearts:{GREEN}     {user_stats.get("heartCount", "N/A")}')
    print(f'{RED}Total Posts:{GREEN}      {user_stats.get("videoCount", "N/A")}')

    # Account Description
    print(f'{RED}\nTitle:{GREEN}            {user_share_meta.get("title", "N/A")}')
    print(f'{RED}Description:{GREEN}      {user_share_meta.get("desc", "N/A")}\n')
    ...
    if recent_videos:
        for idx, video in enumerate(recent_videos, 1):
            video_desc = video.get("desc", "").replace('\n', ' ')
            video_url = f'https://www.tiktok.com/@{user_data.get("uniqueId", "N/A")}/video/{video.get("id", "N/A")}'
            video_stats = video.get("stats", {})
            print(f'{RED}Video {idx}:{GREEN} {video_desc if video_desc else "N/A"}')
            print(f'{RED}Video URL:{GREEN} {video_url}')
            print(f'{RED}Views:{GREEN} {video_stats.get("playCount", "N/A")}')
            print(f'{RED}Likes:{GREEN} {video_stats.get("diggCount", "N/A")}')
            print(f'{RED}Comments:{GREEN} {video_stats.get("commentCount", "N/A")}')
            print(f'{RED}Shares:{GREEN} {video_stats.get("shareCount", "N/A")}\n')

            video_data.append({
                "Description": video_desc,
                "URL": video_url,
                "Views": video_stats.get("playCount", "N/A"),
                "Likes": video_stats.get("diggCount", "N/A"),
                "Comments": video_stats.get("commentCount", "N/A"),
                "Shares": video_stats.get("shareCount", "N/A")
            })
